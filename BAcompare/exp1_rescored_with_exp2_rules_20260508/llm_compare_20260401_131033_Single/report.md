...
- **Single**: P=0.4464, R=0.8621, F1=0.5882 (TP=25, FP=31, FN=4, n=60)
- **Null**: P=0.0000, R=0.0000, F1=0.0000 (TP=0, FP=0, FN=0, n=0)
- **Overall**: P=0.4464, R=0.8621, F1=0.5882 (TP=25, FP=31, FN=4, n=60)

并统计 FP/FN 数量（便于错误类型分析）：

- Multi: FP=0, FN=0
- Single: FP=31.0, FN=4.0
- Null: FP=0, FN=0
- Overall: FP=31.0, FN=4.0
...
Overall 指标为 P=0.4464、R=0.8621、F1=0.5882。其中 Precision 主要受到错误回答（FP）数量影响，Recall 主要受到 Multi/Single 的拒答/信息不足导致的漏答（FN）影响。
...
- **Single**：TP 比例约 41.67%，FP 比例约 51.67%，FN 比例约 6.67%。Single 的 gold 往往是单一事实点；在本实验采用的‘语义裁判’口径下，错误更多表现为给出具体答案但与 gold 语义不一致（FP）。
