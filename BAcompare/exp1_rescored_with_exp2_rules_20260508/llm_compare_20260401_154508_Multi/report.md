...
- **Multi**: P=0.5283, R=0.8000, F1=0.6364 (TP=28, FP=25, FN=7, n=60)
- **Single**: P=0.0000, R=0.0000, F1=0.0000 (TP=0, FP=0, FN=0, n=0)
- **Null**: P=0.0000, R=0.0000, F1=0.0000 (TP=0, FP=0, FN=0, n=0)
- **Overall**: P=0.5283, R=0.8000, F1=0.6364 (TP=28, FP=25, FN=7, n=60)

并统计 FP/FN 数量（便于错误类型分析）：

- Multi: FP=25.0, FN=7.0
- Single: FP=0, FN=0
- Null: FP=0, FN=0
- Overall: FP=25.0, FN=7.0
...
Overall 指标为 P=0.5283、R=0.8000、F1=0.6364。其中 Precision 主要受到错误回答（FP）数量影响，Recall 主要受到 Multi/Single 的拒答/信息不足导致的漏答（FN）影响。

### 9.2 分题型对比（Multi vs Single vs Null）

- **Multi**：TP 比例约 46.67%，FP 比例约 41.67%，FN 比例约 11.67%。Multi 问题通常涉及多步事实或时间先后关系，系统更容易在关系判断上给出错误结论（FP），或在检索不足时输出信息不足（FN）。
- **Single**：TP 比例约 0.00%，FP 比例约 0.00%，FN 比例约 0.00%。Single 的 gold 往往是单一事实点；在本实验采用的‘语义裁判’口径下，错误更多表现为给出具体答案但与 gold 语义不一致（FP）。
- **Null**：TP（正确拒答）比例约 0.00%，FP（未拒答）比例约 0.00%。Null 子集用于度量系统在不可回答问题上的拒答能力。

### 9.3 主要错误来源（基于逐题字段与裁判方法分布）

- **Multi** 的判定来源（judge_method）占比：fast=73.33%, llm=15.00%, abstain=11.67%
其中 `abstain` 表示直接命中拒答规则；`llm` 表示由本地 Ollama 语义裁判给出一致/不一致判定；`fallback` 表示 Ollama 调用失败或输出不可解析时使用的确定性回退规则。

### 9.4 典型案例（从错误样本中引用）

- **Multi-错误回答（FP）示例**（Q2, judge=fast）
  ...
  - Gold: Yes
  - Prediction:  No, Wolfgang did not ask Li Hua about watching "Star Wars: A New Hope" after he asked her about going to see "Overwatch 3". The conversation about Star Wars happened on October 9th, while the conversation about Overwatch 3 happened on January 21st.
