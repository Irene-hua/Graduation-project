...
- **Single**: P=0.5536, R=0.8857, F1=0.6813 (TP=31, FP=25, FN=4, n=60)
- **Null**: P=0.2000, R=1.0000, F1=0.3333 (TP=12, FP=48, FN=0, n=60)
- **Overall**: P=0.4201, R=0.8659, F1=0.5657 (TP=71, FP=98, FN=11, n=180)

并统计 FP/FN 数量（便于错误类型分析）：

- Multi: FP=25.0, FN=7.0
- Single: FP=25.0, FN=4.0
- Null: FP=48.0, FN=0
- Overall: FP=98.0, FN=11.0
...
Overall 指标为 P=0.4201、R=0.8659、F1=0.5657。其中 Precision 主要受到错误回答（FP）数量影响，Recall 主要受到 Multi/Single 的拒答/信息不足导致的漏答（FN）影响。
...
- **Single**：TP 比例约 51.67%，FP 比例约 41.67%，FN 比例约 6.67%。Single 的 gold 往往是单一事实点；在本实验采用的‘语义裁判’口径下，错误更多表现为给出具体答案但与 gold 语义不一致（FP）。
...
- **Single** 的判定来源（judge_method）占比：llm=75.00%, fast=18.33%, abstain=6.67%
