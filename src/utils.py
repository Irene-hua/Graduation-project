"""Utility helpers and common infrastructure for the project.

This module intentionally hosts small cross-cutting tools (audit logger, helpers)
so they can be imported from a single place instead of a dedicated `audit` package.
"""
...
class AuditLogger:
    ...
    def log_query(self, query: str, user: Optional[str] = None, results_count: Optional[int] = None):
        data = {'query': query[:200], 'query_length': len(query)}
        if user:
            data['user'] = user
    ...
    def log_model_invocation(self, model_name: str, inference_time: float, tokens_generated: Optional[int] = None, memory_usage: Optional[float] = None):
        data = {'model_name': model_name, 'inference_time': inference_time}
    ...
    def log_retrieval(self, query: str, top_k: int, retrieval_time: float, num_results: int):
        data = {'query_preview': query[:100], 'top_k': top_k, 'retrieval_time': retrieval_time, 'num_results': num_results}
    ...
    def get_statistics(self) -> Dict:
        stats = {'total_events': 0, 'event_types': {}, 'total_queries': 0, 'total_model_invocations': 0, 'total_errors': 0}
        try:
...
# Provide a short name for external imports
__all__ = ['AuditLogger']
