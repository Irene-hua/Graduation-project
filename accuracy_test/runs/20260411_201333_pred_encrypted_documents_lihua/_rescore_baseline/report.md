...
- **Multi**: P=0.5294, R=0.7500, F1=0.6207 (TP=27, FP=24, FN=9, n=60)
- **Single**: P=0.4500, R=1.0000, F1=0.6207 (TP=27, FP=33, FN=0, n=60)
- **Null**: P=0.1833, R=1.0000, F1=0.3099 (TP=11, FP=49, FN=0, n=60)
- **Overall**: P=0.3801, R=0.8784, F1=0.5306 (TP=65, FP=106, FN=9, n=180)

并统计 FP/FN 数量（便于错误类型分析）：

- Multi: FP=24.0, FN=9.0
- Single: FP=33.0, FN=0
- Null: FP=49.0, FN=0
- Overall: FP=106.0, FN=9.0
...
Overall 指标为 P=0.3801、R=0.8784、F1=0.5306。其中 Precision 主要受到错误回答（FP）数量影响，Recall 主要受到 Multi/Single 的拒答/信息不足导致的漏答（FN）影响。

### 9.2 分题型对比（Multi vs Single vs Null）

- **Multi**：TP 比例约 45.00%，FP 比例约 40.00%，FN 比例约 15.00%。Multi 问题通常涉及多步事实或时间先后关系，系统更容易在关系判断上给出错误结论（FP），或在检索不足时输出信息不足（FN）。
- **Single**：TP 比例约 45.00%，FP 比例约 55.00%，FN 比例约 0.00%。Single 的 gold 往往是单一事实点；在本实验采用的‘语义裁判’口径下，错误更多表现为给出具体答案但与 gold 语义不一致（FP）。
- **Null**：TP（正确拒答）比例约 18.33%，FP（未拒答）比例约 81.67%。Null 子集用于度量系统在不可回答问题上的拒答能力。

### 9.3 主要错误来源（基于逐题字段与裁判方法分布）

- **Multi** 的判定来源（judge_method）占比：fast=66.67%, llm=18.33%, abstain=15.00%
- **Single** 的判定来源（judge_method）占比：llm=81.67%, fast=18.33%
- **Null** 的判定来源（judge_method）占比：not_abstain=81.67%, abstain=18.33%
